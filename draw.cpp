...
void cdraw::Draw_Text(float x, float y, float size, float *colour, int align, LPSTR font, const char *txtformat, ...)
{
	char szBuffer[512] = "";
	va_list	ap;
	...
	int sztext = strlen(szBuffer)*10*size;
	if (align == F_CENTER)
		x = x-(sztext/2);
...
void cdraw::Draw_Box(float x, float y, float width, float height, float *colour, float rotate)
{
	trap_R_DrawRotatedPic(x, y, width, height, 0, 0, 0, 1, rotate, colour, bot.shaders.White);
}

void cdraw::Draw_Gradient(float x, float y, float width, float height, float *colour1, float *colour2, bool horizontal)
{
...
void cdraw::Draw_Shader(float x, float y, float width, float height, float *colour, LPSTR shader)
{
	trap_R_DrawStretchPic(x, y, width, height, 0, 0, 1, 1, colour, shader);
}

void cdraw::Draw_ShaderRotate(float x, float y, float width, float height, float *colour, LPSTR shader, float rotate)
{
	trap_R_DrawRotatedPic(x, y, width, height, 0, 0, 1, 1, rotate, colour, shader);
}

void cdraw::Draw_Border(float x, float y, float width, float height, float size, float *colour)
{
	Draw_Box(x, (y-size), (width+size), size, colour); 
	Draw_Box((x+width), (y-size), size, (height+2*size), colour); 
	Draw_Box((x-size), (y+height), (width+2*size), size, colour);
	Draw_Box((x-size), (y-size), size, (height+2*size), colour);
}

void cdraw::Draw_Line(float startX, float startY, float endX, float endY, float *colour, float width)
{
...
void  cdraw::render_GUI_CheckBox(float x, float y, float w, float h, float *color)
{
	...
	Draw_Border(x, y, w, h, 1, colorBlack);
	Draw_Border(x+3, y+3, w-6, h-6, 1, colorBlack);
...
void  cdraw::render_GUI_Menu(float x, float y, float w, float h, float *color)
{
	Draw_Box(x, y, w, h, color);
	Draw_Border(x, y, w, h,1, colorBlack);
	Draw_Border(x+2, y+2, w-4, h-4, 1, colorBlack);
}
