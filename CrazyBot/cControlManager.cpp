...
void Reset()
{
	...
	print::Say("^2[^7Crazy^9Bot^3:ad^2]^7: Settings reset...");
}
...
void cMenuManager::initialize()
{
	...
	cMenu* mainMenu = new cMenu("CrazyBot by BratokHR", 10, 100, 450, 400,STYLE_CLOSE | STYLE_MINIMIZE); 
	...
	AimbotTab->add(new CCheckbox("Aimbot", 30, POS(0), &cFuncAimbot.bValue));
	AimbotTab->add(new CCheckbox("Aim key", 30, POS(1), &cFuncAimKeyBool.bValue));
	AimbotTab->add(new CSlider("Offset X", 30, POS(4), 0, 100, &cFuncAimOffsetX.iValue));
	AimbotTab->add(new CSlider("Offset Y", 30, POS(6), 0, 100, &cFuncAimOffsetY.iValue));
	AimbotTab->add(new CFSlider("Speed aiming [Human]", 250, POS(6), 1.0f, 100.0f, &cFuncAimSpeed.fValue));
	AimbotTab->add(new CSlider("Fov", 250, POS(4), 1, 360, &cFuncAimFov.iValue));

	cControlManager* DropAim = new CDropDown("Type", 250, POS(1)-5, &cFuncAimType.iValue);
	  DropAim->addChoice("Auto aim");
	  DropAim->addChoice("Human aim");
	  DropAim->addChoice("Silent aim");
	 AimbotTab->add(DropAim);

	 cControlManager* DropFilter = new CDropDown("Filter", 250, POS(8), &cFuncAimFilter.iValue);
	 ...
	 cControlManager* DropBone = new CDropDown("Bone tag", 30, POS(8), &cFuncAimBoneTag.iValue);
	...
	ESPTab->add(new CCheckbox("Names", 30, POS(0), &cFuncESPNames.bValue));
	ESPTab->add(new CCheckbox("2D Box", 30, POS(1), &cFuncESP2Dbox.bValue));
	ESPTab->add(new CCheckbox("3D Box", 30, POS(2), &cFuncESP3Dbox.bValue));
	ESPTab->add(new CCheckbox("Distance", 30, POS(3), &cFuncESPDistance.bValue));
	ESPTab->add(new CCheckbox("Lines", 30, POS(4), &cFuncESPLines.bValue));
	ESPTab->add(new CCheckbox("Skeleton", 30, POS(5), &cFuncESPSkeleton.bValue));
	...
	VisualTab->add(new CCheckbox("Wallhack", 30, POS(0), &cFuncVisualWH.bValue));
	VisualTab->add(new CCheckbox("Grenade indicator", 30, POS(1), &cFuncVisualGranade.bValue));
	VisualTab->add(new CCheckbox("MG indicator", 30, POS(2), &cFuncVisualMG.bValue));
	VisualTab->add(new CCheckbox("Radar", 30, POS(3), &cFuncRadar.bValue));
	VisualTab->add(new CCheckbox("Target info", 30, POS(4), &cFuncVisualTargetInfo.bValue));
	...
	cControlManager* VisualCrossFLTR = new CDropDown("Cross", 220, POS(0), &cFuncVisualCross.iValue);
	  ...
	  VisualCrossFLTR->addChoice("Cross small");
	  VisualCrossFLTR->addChoice("Cross big");
	 VisualTab->add(VisualCrossFLTR);

	cMenu* MiscTab = new cMenu("Misc", -1, -1, 300, 160, NULL);
	MiscTab->add(new CGroupBox("Misc", 6, 35, 437, 360));

	MiscTab->add(new CCheckbox("NoRecoil", 30, POS(0), &cFuncMiscNoRecoil.bValue));
	MiscTab->add(new CCheckbox("AutoShoot", 30, POS(1), &cFuncAutoShoot.bValue));
	MiscTab->add(new CCheckbox("NameStealer", 30, POS(2), &cFuncMiscNameStealer.bValue));
	MiscTab->add(new CCheckbox("KillSound", 30, POS(3), &cFuncMiscKillSound.bValue));

	MiscTab->add(new CButton("Open console", 250, POS(13), 160, 20, (void*)bot.OpenConsole));
	MiscTab->add(new CButton("Reset settings", 250, POS(15), 160, 20, (void*)Reset));
...
void cMenuManager::drawMenus()
{
	...
	{
		int width = bot.cg->refdef.width;
		int height = bot.cg->refdef.height;
		Draw.Box(0, 0, width+1, height+1, colorBlack);// фон при выпадении MessegeBox
	}
...
{
	int width = 70;
	Draw.Shader(mouseX-(width/2), mouseY-(width/2)+1, width, width, clCursor, bot.shaders.Cursor);
}
...
void cMenuManager::drawMenu(cMenu* menu)
{
	...
	if ((titlebarOffset <= 2 || menu->minimized || menu->style & STYLE_TRANSPARENT) && menuOpen)
		Draw.Border(menu->xPosition , menu->yPosition, menu->width, titlebarHeight, 1, colorBlack);// рамка заголовка
	if ((menuOpen || menu->locked) && ~menu->style & STYLE_TRANSPARENT)//{}
		Draw.Border(menu->xPosition, menu->yPosition + titlebarHeight + titlebarOffset, menu->width, menu->height, 1, colorBlack);// рамка основной поверхности
	...
	Draw.Box(menu->xPosition, menu->yPosition, menu->width, titlebarHeight, titleBarColour);//заголовок
	Draw.Text(menu->xPosition+5, menu->yPosition+titlebarHeight+1, 1, titleBarColourText, F_LEFT, bot.fonts.Big, menu->title.c_str());
}
...
void cMenuManager::handleInput()
{
	...
	if (GetAsyncKeyState(menuKey) & 1)
	{
		if (!menuOpen) 
		{
			Hook->MemoryWrite((void*)bot.MenuOpen, (void*)"\x74\x7C", 2);
			Hook->MemoryWrite((void*)bot.Attack, (void*)"\xC3", 1);
			menuOpen = true;
		}
		else
		{
			SetCursorPos((bot.cg->refdef.width/2),(bot.cg->refdef.height/2));
			Hook->MemoryWrite((void*)bot.MenuOpen, (void*)"\x75\x7C", 2);
			Hook->MemoryWrite((void*)bot.Attack, (void*)"\x56", 1);
			GSettings.SaveSettings();
			print::Say("^2[^7Crazy^9Bot^3:ad^2]^7: Settings saved...");
			menuOpen = false;
		}
		char sound[MAX_PATH];
	...
	if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
	{
		...
		if (controlState == STATE_HOVER)
		{
			...
			if (control->controlType == TYPE_SLIDER)
			{
				...
				*(int*)control->value = (mouseX - x) * ((int)control->max - (int)control->min) / control->width;

				if (*(int*)control->value > (int)control->max) *(int*)control->value = (int)control->max;
				if (*(int*)control->value < (int)control->min) *(int*)control->value = (int)control->min;
	...
	for (UINT i = 0; i < mL.size(); i++)
	{
		...
		for (UINT j = 0; j < menu->cL.size(); j++)
		{
			...
			if (control->tL.size() > 0)
			{
				...
				for (UINT k = 0; k < activeTab->cL.size(); k++)
				{
...
{
	for (UINT i = 0; i < activeControl->tL.size(); i++)
	{
		int w = 107;
		int h = 20;
...
void cMenuManager::drawControls(cMenu* menu)
{
	for (UINT i = 0; i < menu->cL.size(); i++)
	{
		...
		case TYPE_SCOLOR:
			...
			break;
		case TYPE_TAB:
			for (UINT i = 0; i < control->tL.size(); i++)
			{
				...
				if (i == control->activeTab)
					Draw.Box(x + 1, y + 1, w - 1, h - 1, clCheckOK);
				else
					Draw.Box(x + 1, y + 1, w - 1, h - 1, clTab);
				Draw.Border(x, y, w, h, 1, colorBlack);

				Draw.Text(x+2, y+h, 0.8, MainText, F_LEFT, bot.fonts.Big, tab->title.c_str());
			}
...
void cMenuManager::drawLabel(cControlManager* control)
{
	...
	Draw.Text(x, y+8, 0.8, MainText, F_LEFT, bot.fonts.Big, control->label.c_str());
}
...
void cMenuManager::drawGroupBox(cControlManager* control)
{
	...
	float x = menu->xPosition + control->xOffset;
	float y = menu->yPosition + control->yOffset + titlebarHeight + titlebarOffset;

	int stringLength = strlen(control->label.c_str()) * 11;

	Draw.Box(x, y, 1, (float)control->height, clGroup);
	Draw.Box(x, y+control->height, (float)control->width, 1, clGroup);
	Draw.Box(x+control->width, y, 1, (float)control->height+1, clGroup);
	Draw.Box(x, y, 18, 1, clGroup);
	Draw.Box(x+10+stringLength, y, (float)control->width-10-stringLength, 1, clGroup);

	Draw.Text(x+20, y+8, 0.8, MainText, F_LEFT, bot.fonts.Big, control->label.c_str());
}
...
void cMenuManager::drawButton(cControlManager* control)
{
	...
	Draw.Text(x+5, y+control->height, 0.8, MainText, F_LEFT, bot.fonts.Big, control->label.c_str());
}
...
void cMenuManager::drawCheckbox(cControlManager* control)
{
	...
	Draw.Text(x + control->width + 5, y+control->height, 0.8, colorWhite, F_LEFT, bot.fonts.Big, control->label.c_str());
	if (control->value && *(bool*)control->value == true)
		Draw.Box(x + 4, y + 4, control->width - 7, control->height - 7, clCheckOK);
}
...
void cMenuManager::drawDropDown(cControlManager* control)
{
	...
	if (state == STATE_NORMAL)
		Draw.Box(x, y, control->width, control->height, clDrop);
	else Draw.Box(x, y, control->width, control->height, clCheckAct);

	Draw.Border(x, y, control->width, control->height, 1, colorBlack);

	if (control->value)
	{
		Draw.Text(x+5, y+control->height, 0.8, MainText, F_LEFT, bot.fonts.Big, control->cL[*(int*)control->value].c_str());
		Draw.Text(x, y, 0.8, MainText, F_LEFT, bot.fonts.Big, control->label.c_str());

		if (control->isDropped)
		{
			Draw.ShaderRotate(x+control->width-20, y+(control->height/2)-7, 15, 15, colorBlack, bot.shaders.Arrow, 0);
			short choiceListSize = control->cL.size();
			for (int i = 0; i<choiceListSize; ++i)
			{
				int yOffset = (i + 1)*control->height;
				Draw.Box(x+1, y+1+yOffset, control->width-1, control->height, clDrop);
				Draw.Text(x+5, y+control->height+yOffset, 0.8, MainText, F_LEFT, bot.fonts.Big, control->cL[i].c_str());
			}
		}
		else Draw.ShaderRotate(x+control->width-20, y+(control->height/2)-7, 15, 15, colorWhite, bot.shaders.Arrow, 180);
	}
}
...
void cMenuManager::drawSlider(cControlManager* control)
{
	...
	Draw.Box(x, y+(control->height/2), control->width, control->height/4, clSliderLine);

	int valueX = x + ((*(int*)control->value - control->min) * control->width / (control->max - control->min));
	Draw.Box(valueX, y, 4, control->height, clSliderBox);

	Draw.Text(x, y+control->height-10, 0.8, MainText, F_LEFT, bot.fonts.Big, control->label.c_str());
	Draw.Text(x+control->width+7, y+control->height+1, 0.8, MainText, F_LEFT, bot.fonts.Big, "%i", *(int*)control->value);
}
void cMenuManager::drawFSlider(cControlManager* control)
{
	...
	float valueX = x + ((*(float*)control->value - control->min) * control->width / (control->max - control->min));
	Draw.Box(valueX, y, 4, control->height, clSliderBox);

	Draw.Text(x, y+control->height-10, 0.8, MainText, F_LEFT, bot.fonts.Big, control->label.c_str());
	Draw.Text(x+control->width+7, y+control->height+1, 0.8, MainText, F_LEFT, bot.fonts.Big, "%.1f", *(float*)control->value);
...
void cMenuManager::drawSelectColor(cControlManager* control)
{
	...
	Draw.Box(x, y, control->width, control->height, color);
}
...
void cMenuManager::centerMenu(cMenu* menu)
{
	int w = menu->width;
	int h = menu->height;
	int centerX = (bot.cg->refdef.width / 2) - (w / 2);
	int centerY = (bot.cg->refdef.height / 2) - (h / 2);
